...
var models_folder = "D:/work/dev/projects/data_dumps/schema_models/csv/processed/"
	...
	require('fs').readdir(models_folder, function(err, items) {
		for (var i=0; i<items.length; i++) {
			...
			if (item.indexOf("json")>0){
				...
				var config_file_to_add = models_folder+"/"+items[i]
				var tmp_new_model = JSON.parse(fs.readFileSync(config_file_to_add, 'utf8'));
				
				tmp_new_model[tmp_new_model.name] = {}//JS'{"dataSource": "db","public": true}'
	tmp_new_model[tmp_new_model.name].dataSource = "db" 
	tmp_new_model[tmp_new_model.name].public = true 
